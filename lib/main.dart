...
class VisionRT extends StatefulWidget {
  const VisionRT({Key? key, this.onScanned}) : super(key: key);
...
class _VisionRT extends State<VisionRT> {
  ...
  AppleVisionRecognizeTextController visionController =
  ...
  startReading() {
    camera.startLiveFeed((InputImage i) {
      ...
      if (mounted) {
        Uint8List? image = i.bytes;
        visionController
            .processImage(RecognizeTextData(
          image: image!,
          imageSize: imageSize,
          recognitionLevel: RecognitionLevel.accurate,
          languages: [const Locale('en-US')],
          ...
          textData = data;
          if (textData != null) {
            log(textData!.map((e) => e.listText).toString());
          }
          setState(() {});
        });
      }
    ...
    return Stack(
        children: <Widget>[
              SizedBox(
                  ...
                  child: loading
                      ? Container()
                      : CameraSetup(camera: camera, size: imageSize)),
              Align(
  ...
  List<Widget> showRects() {
    ...
    for (int i = 0; i < textData!.length; i++) {
      ...
      // log("Text: ${textData![i].listText[0]}");
    }
