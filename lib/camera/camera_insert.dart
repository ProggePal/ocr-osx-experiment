...
class InsertCamera {
    ...
    try {
      if (Platform.isMacOS) {
        var value = await CameraMacOSPlatform.instance.initialize(
            deviceId: usedCameraId, //'F4B311A6-553F-4B69-B66B-7A5C00000001',
            audioDeviceId: null,
  ...
  Future toggleMirrored() async {
    if (Platform.isMacOS) {
      isMirrored = !isMirrored;
      controllerMacos!.setVideoMirrored(isMirrored);
      stopFeed(0);
      if (isLive) {
...
class CameraSetup extends StatefulWidget {
  const CameraSetup(
      {Key? key, required this.camera, this.size = const Size(640, 480)})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(children: [
      ...
      ElevatedButton(
          onPressed: () {
            //print ids
            print(widget.camera._camerasMacos
                .map((e) => e.deviceId)
                .toList()
                .join(','));

            var index = widget.camera._camerasMacos
                .indexWhere((c) => c.deviceId == widget.camera.usedCameraId);

            if (index == -1) {
              widget.camera.usedCameraId =
                  widget.camera._camerasMacos[0].deviceId;
            } else {
              var nextIndex = index + 1;
              if (nextIndex == widget.camera._camerasMacos.length) {
                nextIndex = 0;
              }
              widget.camera.usedCameraId =
                  widget.camera._camerasMacos[nextIndex].deviceId;
            }
            setState(() {
              widget.camera.switchCamera(index);
            });
          },
